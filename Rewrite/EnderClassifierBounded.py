...
class EnderClassifier(BaseEstimator, ClassifierMixin):
    ...
    # def compute_kmeans_equivalent_points_lower_bound(self, X_subset, y_subset, lambda_reg=0.01, max_clusters=5):
    #     """
    #     Compute the k-Means Equivalent Points Lower Bound for a partial rule's covered data.

    #     Parameters:
    #     - X_subset: 2D numpy array (n_samples, n_features), samples covered by current rule
    #     - y_subset: 1D numpy array (n_samples,), corresponding labels
    #     - lambda_reg: float, regularization parameter
    #     - max_clusters: int, maximum number of clusters (leaves) to consider

    #     Returns:
    #     - lower_bound: float, estimated lower bound on achievable loss from this rule path
    #     """
    #     # Step 1: Group by equivalent feature vectors
    #     equivalence_map = defaultdict(list)
    #     for xi, yi in zip(X_subset, y_subset):
    #         key = tuple(xi)
    #         equivalence_map[key].append(yi)

    #     # Step 2: Compute weighted points (means and counts)
    #     y_means = []
    #     weights = []
    ...
    #     # Step 3: Solve weighted 1D k-means
    #     min_total_loss = self.weighted_1d_kmeans(y_means, weights, max_clusters)

    #     # Step 4: Correction for transforming to weighted points
    #     original_total = np.sum(y_subset ** 2)
    #     compressed_total = np.sum([w * ym ** 2 for w, ym in zip(weights, y_means)])
    #     correction = original_total - compressed_total

    #     # Final bound (includes correction + regularization)
    #     return min_total_loss + lambda_reg * max_clusters + correction
    
    # def weighted_1d_kmeans(self, y_means, weights, max_clusters):
    #     """
    #     Computes the optimal weighted 1D k-means loss for up to max_clusters clusters.
        
    #     Parameters:
    #     - y_means: List or array of means (1D target values for equivalent sets).
    #     - weights: List or array of weights (number of samples per mean).
    #     - max_clusters: Maximum number of clusters to consider.
        
    #     Returns:
    #     - best_total_loss: Minimum total weighted k-means loss.
    #     """
    #     import numpy as np
    ...
    #     # Prefix sums for efficient segment statistics
    ...
    #         prefix_y[i + 1] = prefix_y[i] + weights[i] * y_means[i]
    #         prefix_y2[i + 1] = prefix_y2[i] + weights[i] * y_means[i] ** 2
    ...
    #         if w == 0:
    #             return 0
    #         y_sum = prefix_y[j] - prefix_y[i]
    #         y2_sum = prefix_y2[j] - prefix_y2[i]
    #         mean = y_sum / w
    #         return y2_sum - 2 * mean * y_sum + w * mean ** 2
    ...
    #             for i in range(j):
    #                 dp[k][j] = min(dp[k][j], dp[k - 1][i] + segment_cost(i, j))

    #     # Best loss for any cluster count
    #     return np.min(dp[1:max_clusters + 1, n])
    
    # def compute_current_ensemble_risk(self) -> float:
    #     """
    #     Computes the current empirical risk of the ensemble of completed rules.
    #     Returns:
    #     - empirical_risk: float, the empirical risk of the ensemble
    #     """
    #     empirical_risk = 0.0
    #     for i in range(len(self.X)):
    #         if self.covered_instances[i] >= 0:
    #             for k in range(self.num_classes):
    #                 empirical_risk += INSTANCE_WEIGHT * self.probability[i][k] * (
    #                     self.value_of_f[i][k] - self.y[i]) ** 2
    #     return empirical_risk / len(self.X)

    def weighted_1d_kmeans(self, y_means, weights, max_clusters):
        """
        Computes optimal weighted 1D k-means loss using dynamic programming.
        """
        n = len(y_means)
        y_means = np.array(y_means)
        weights = np.array(weights)
    ...
    def compute_rule_lower_bound(self, X_subset, residuals, lambda_reg=0.0, max_clusters=4):
        """
        ...
        return kmeans_loss + lambda_reg * max_clusters + correction
    
    def get_best_rule_loss(self) -> float:
        """
        Iterates over all rules and finds which one has the lowest empirical risk.
        Returns:
        ...
        for rule in self.rules:
            ...
            for i in range(len(self.X)):
                for k in range(self.num_classes):
                    empirical_risk += INSTANCE_WEIGHT * self.probability[i][k] * (
                        rule.decision[k] - self.y[i]) ** 2
            empirical_risk /= len(self.X)
            if empirical_risk < best_rule_loss:
                best_rule_loss = empirical_risk
        return best_rule_loss
    ...
    def create_rule(self):
        ...
        while creating:
            ...
            if best_attribute == -1 or not best_cut.exists:
                ...
                best_rule_loss = self.get_best_rule_loss()
    ...
    def initialize_for_rule(self):
        ...
        self.instances_covered_by_current_rule = np.zeros(len(self.X), dtype=int)
